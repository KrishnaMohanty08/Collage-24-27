...
void do_receive(int sockfd, struct sockaddr_in *server_addr) {
    char charBuffer[100];
    int flags = 0;
...
int main() {
    ...
    while (1) {
        int choice;
        printf("\n[Client] Enter 1 to receive, 2 to send, 0 to exit: ");
        scanf("%d", &choice);

        if (choice == 1)
            do_receive(sockfd, &server_addr);
        else if (choice == 2)
